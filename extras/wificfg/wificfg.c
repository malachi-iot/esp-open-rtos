                                 ...
                                 char *buf, size_t len)
{
    ...
    if (method != HTTP_METHOD_HEAD) {
        ...
        if (hostname) {
            snprintf(buf, len, "<dt>Hostname</dt><dd>");
        ...
        if (wificfg_write_string(s, "<dt>LwIP version</dt><dd>" LWIP_VERSION_STRING "</dd>") < 0) return;
        if (wificfg_write_string(s, "<dt>FreeRTOS version</dt><dd>" tskKERNEL_VERSION_NUMBER "</dd>") < 0) return;
        if (wificfg_write_string(s, "<dt>Newlib version</dt><dd>" _NEWLIB_VERSION "</dd>") < 0) return;
                                ...
                                char *buf, size_t len)
{
    ...
    if (method != HTTP_METHOD_HEAD) {
        ...
        if (wificfg_write_string(s, http_wifi_station_content[10]) < 0) return;
    }
}
...
void wificfg_init(uint32_t port, const wificfg_dispatch *dispatch)
{
    ...
    xTaskCreate(server_task, "WiFi Cfg HTTP", 448, params, 2, NULL);
}
